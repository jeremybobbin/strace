...
"\
  ...
  --absolute-timestamps=[[format:]FORMAT[,[precision:]PRECISION]]\n\
     ...
     format:     none, time, unix, iso8601, or strftime=STRING; default is time\n\
...
{
	static struct xlat_data intr_str[] = {
...
init(int argc, char *argv[])
{
	...
	static const char *ttflag_str = "format:time,precision:us";
...
# error Bug in DEFAULT_QUAL_FLAGS
	...
	static const struct option longopts[] = {
		...
		{ "decode-fds",	optional_argument, 0, GETOPT_QUAL_FD },
	...
	if (yflag_short) {
		if (decode_fd_set_updated) {
			error_msg_and_die("-y and -e decode-fds/--decode-fds"
